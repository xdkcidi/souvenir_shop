document.addEventListener("DOMContentLoaded", function () {
  ...
  if (tabs.length > 0) {
    ...
    if (!tabToOpen && window.location.hash) {
      ...
      if (["profile", "orders", "favorites", "coupons", "security"].includes(h))
        tabToOpen = h;
    }

    if (
      tabToOpen &&
      ["profile", "orders", "favorites", "coupons", "security"].includes(
        tabToOpen,
      )
    ) {
      showTab(tabToOpen);
  ...
  const favTabBadge = document.querySelector(
    '.account-tab[data-tab="favorites"] .badge',
  );
  ...
  function setFavCount(n) {
    ...
    if (favoritesCountDesc)
      favoritesCountDesc.textContent = "Товаров в избранном: " + String(n);
  }
  ...
  async function favList() {
    const res = await fetch(FAV_API + "?action=list", {
  ...
  document.addEventListener("click", async (e) => {
    ...
    const productDbId = btn.getAttribute("data-product-db-id");
