...
renderHead(
    'Заказы — Админка',
    'Административная страница заказов Лавки: фильтрация, просмотр состава заказа и управление статусами.',
    [
        'css/style.css',
        'css/admin.css'
    ]
      ...
      <div class="filters">
        <div>
          ...
          <input class="input" id="q" name="q" type="text"
            value="<?= h($q) ?>"
            placeholder="ID, имя, телефон, email, адрес, промокод">
        </div>
        ...
        <div>
          ...
          <select class="input" id="status" name="status">
            ...
            <option value="new" <?= $status === 'new' ? 'selected' : '' ?>>Новый
            </option>
            <option value="processing" <?= $status === 'processing' ? 'selected' : '' ?>>В
              обработке</option>
            <option value="shipped" <?= $status === 'shipped' ? 'selected' : '' ?>>Отправлен
            </option>
            <option value="completed" <?= $status === 'completed' ? 'selected' : '' ?>>Завершён
            </option>
            <option value="cancelled" <?= $status === 'cancelled' ? 'selected' : '' ?>>Отменён
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          <select class="input" id="payment_method" name="payment_method">
            <option value="">Все</option>
            <option value="card" <?= $payment === 'card' ? 'selected' : '' ?>>Картой
            </option>
            <option value="cash" <?= $payment === 'cash' ? 'selected' : '' ?>>Наличными
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          <select class="input" id="delivery_type" name="delivery_type">
            <option value="">Все</option>
            <option value="pickup" <?= $deliveryType === 'pickup' ? 'selected' : '' ?>>Самовывоз
            </option>
            <option value="delivery" <?= $deliveryType === 'delivery' ? 'selected' : '' ?>>Доставка
            </option>
          </select>
        </div>
        ...
        <div class="full-actions">
          ...
          <a class="btn btn--outline" href="../pages/admin_orders.php">Сбросить</a>
          <span style="color:#666;">Найдено заказов:
            <?= count($orders) ?></span>
        </div>
      </div>
    </form>
        ...
        <div style="display:grid; gap:14px;">
          <div class="info-grid">
            <div class="info-box">
              ...
              <div class="meta-line">Имя:
                <?= h($order['customer_name']) ?: '—' ?>
              </div>
              <div class="meta-line">Телефон:
                <?= h($order['phone']) ?: '—' ?>
              </div>
              <div class="meta-line">Email:
                <?= h($order['email']) ?: '—' ?>
              </div>
            </div>
            ...
            <div class="info-box">
              ...
              <div class="meta-line">Способ получения:
                <?= h(deliveryLabel($order['delivery_type'])) ?>
              </div>
              <div class="meta-line">Способ оплаты:
                <?= h(paymentLabel($order['payment_method'])) ?>
              </div>
              <div class="meta-line">Стоимость доставки:
                <?= formatPrice((float)($order['delivery_fee'] ?? 0)) ?>
              </div>
              <div class="meta-line">Слот:
                <?= h($order['delivery_slot']) ?: '—' ?>
              </div>
            </div>

            <div class="info-box">
              <strong>Адрес</strong>
              <div class="meta-line">Адрес доставки:
                <?= h($order['delivery_address']) ?: '—' ?>
              </div>
              <div class="meta-line">Адрес самовывоза:
                <?= h($order['pickup_address']) ?: '—' ?>
              </div>
              <div class="meta-line">Город:
                <?= h($order['city']) ?: '—' ?>
              </div>
              <div class="meta-line">
                Улица / дом / кв.:
              ...
              <div class="meta-line">Подъезд / доп. инфо:
                <?= h($order['entrance_info']) ?: '—' ?>
              </div>
            </div>

            <div class="info-box">
              <strong>Суммы и скидки</strong>
              <div class="meta-line">Сумма товаров:
                <?= formatPrice((float)($order['items_sum'] ?? 0)) ?>
              </div>
              <div class="meta-line">Скидка:
                <?= (int)($order['discount_percent'] ?? 0) ?>%
              </div>
              <div class="meta-line">Сумма скидки:
                <?= formatPrice((float)($order['discount_sum'] ?? 0)) ?>
              </div>
              <div class="meta-line">Итого:
                <?= formatPrice((float)($order['total_sum'] ?? 0)) ?>
              </div>
              <div class="meta-line">Промокод:
                <?= h($order['promo_code']) ?: '—' ?>
              </div>
            </div>
          </div>
        ...
        <div style="display:grid; gap:14px;">
          <div class="items-box">
            ...
            <div class="summary-row">
              <span class="summary-chip">Позиций:
                <?= count($items) ?></span>
              <span class="summary-chip">Итого:
                <?= formatPrice((float)($order['total_sum'] ?? 0)) ?></span>
            </div>
          </div>

          <div class="status-box">
            <strong>Изменить статус</strong>
            <form class="status-form" action="../php/admin_order_status.php" method="post">
              <input type="hidden" name="id" value="<?= $orderId ?>">

              <select class="input" name="status" required>
                <option value="new" <?= $order['status'] === 'new' ? 'selected' : '' ?>>Новый
                </option>
                <option value="processing" <?= $order['status'] === 'processing' ? 'selected' : '' ?>>В
                  обработке</option>
                <option value="shipped" <?= $order['status'] === 'shipped' ? 'selected' : '' ?>>Отправлен
                </option>
                <option value="completed" <?= $order['status'] === 'completed' ? 'selected' : '' ?>>Завершён
                </option>
